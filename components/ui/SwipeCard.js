...
export default function SwipeCard({ event, onSwipe }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="rounded-3xl bg-navy/90 p-6 sm:p-8 border border-white/10 backdrop-blur">
          <div className="flex items-center justify-between gap-2">
            <p className="text-xs uppercase tracking-[0.2em] text-mint">Evento social</p>
            <span className="px-3 py-1 rounded-full bg-white/10 text-xs text-gray-200 border border-white/10">{event.location}</span>
          </div>
          <h2 className="mt-3 text-2xl sm:text-3xl font-semibold text-white leading-tight">{event.title}</h2>
